...
from kivy.uix.spinner import Spinner  # Para seleccionar cuentas
...
# Ruta para el archivo del libro de cuentas
...
# Cargar o inicializar el libro de cuentas
...
class ContableApp(App):
    def build(self):
        ...
        self.cuenta_spinner = Spinner(text='Selecciona una cuenta', values=self.obtener_cuentas(), size_hint_y=None, height=40)
        input_layout.add_widget(self.cuenta_spinner)
        ...
        # Layout para agregar nuevas cuentas
        cuenta_layout = GridLayout(cols=2, row_default_height=40, spacing=10, size_hint_y=None)
        cuenta_layout.bind(minimum_height=cuenta_layout.setter('height'))

        cuenta_layout.add_widget(Label(text='Nueva Cuenta:'))
        self.nueva_cuenta_input = TextInput(hint_text='Nombre de la nueva cuenta', multiline=False)
        cuenta_layout.add_widget(self.nueva_cuenta_input)

        cuenta_layout.add_widget(Label(text='Descripción:'))
        self.nueva_descripcion_input = TextInput(hint_text='Descripción de la nueva cuenta', multiline=False)
        cuenta_layout.add_widget(self.nueva_descripcion_input)

        add_cuenta_button = Button(text='Agregar Cuenta', size_hint_y=None, height=40)
        add_cuenta_button.bind(on_press=self.agregar_cuenta)
        ...
        layout.add_widget(cuenta_layout)
        layout.add_widget(add_cuenta_button)
    ...
    def agregar_cuenta(self, instance):
        cuenta = self.nueva_cuenta_input.text.strip()
        descripcion = self.nueva_descripcion_input.text.strip()

        if cuenta and descripcion:
            agregar_cuenta(cuenta, descripcion)
            self.cuenta_spinner.values = self.obtener_cuentas()  # Actualizar las cuentas en el Spinner
            self.nueva_cuenta_input.text = ''
            self.nueva_descripcion_input.text = ''
            self.result_label.text = "Cuenta agregada exitosamente."
        else:
            self.result_label.text = "Por favor, complete ambos campos para agregar una cuenta."
    ...
    def actualizar_resultados(self):
        ...
        # Mostrar las cuentas existentes en el libro de cuentas
        if not libro_cuentas.empty:
